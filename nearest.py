...
def run_detection():
    ...
    previous_vector_x = 0  # 保存上一次的移动向量x
    previous_vector_y = 0  # 保存上一次的移动向量y
    moved_count = 0  # 记录反向移动次数
    max_moved_count = 8  # 最大反向移动次数
    ...
    while controlling_mouse:
        ...
        if closest_box_info:
            ...
            moved_count = 0  # 重置反向移动计数器
        else:
            # 当没有检测到目标时，移动上一次向量的反方向
            if previous_vector_x != 0 or previous_vector_y != 0:
                if moved_count < max_moved_count:
                    move_mouse_by(-previous_vector_x, -previous_vector_y)
                    moved_count += 1  # 增加反向移动计数器

def move_mouse_by(delta_x, delta_y):
    driver.move_R(int(delta_x), int(delta_y))
